...
(threads <- purrr::map_dfr(section_suffix, ~{
  get_threads(suffix = .x, thread_start_date = lubridate.:today()-days(7))
  }) %>% 
  dplyr::filter(stringr::str_detect(name, stringr::regex(covid_keywords, ignore_case = TRUE))) %>% 
  dplyr::pull(link))
```

Now, the threads can be scraped (disclaimer: because they have to be scraped from anew every time this document is knitted, I will just exemplify the process by scraping one of them):

```{r}
(covid_content <- purrr::map_dfr(threads[1], scrape_thread))
```
